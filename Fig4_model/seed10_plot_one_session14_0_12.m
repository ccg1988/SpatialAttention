...
seed = 10 ; rng(seed, "twister"); 
% 6+3 generators: https://www.mathworks.com/help/matlab/ref/rng.html
norm_sgm_HP = [14] ;
norm_drift_peak_HP = [1] ; % the maximum drift
attn_drift_peak_HP = [12] ; % the maximum drift
...
attn_drift_all = nan(length(stim_amp_drift), length(norm_drift), length(attn_drift));
...
attn_drift_all(s, n, a) = attn_drift(a) ;
% 1) example: largest firing rate (89spikes/s) in the top-most; 
% 2) example: largest passive than attention firing rate in the bottom-right;
% 3) example: firing rate on the diagonal (i.e., hits==passive)
% 4) example: 
% if peak_activity_in(s, n, a)>85 
% if (peak_activity_in(s, n, a)>1.4)&&(peak_activity_in(s, n, a)<1.5)&&(peak_activity_out(s, n, a)>26)
% if (round(peak_activity_in(s, n, a))==24)&&(round(peak_activity_out(s, n, a))==27)
% if (round(peak_activity_in(s, n, a))==2)&&(round(peak_activity_out(s, n, a))==1)    
if round(attn_drift(a))==10 % >=5 is empty
%     AA = AA + 1;
%     disp(['In=',num2str(peak_activity_in(s, n, a)),' Out=',num2str(peak_activity_out(s, n, a))])
%     pos=get(0,'ScreenSize'); X_size=pos(3);Y_size=pos(4);
%     f = figure('position',[X_size*0.05 Y_size*0.1 X_size*0.45 Y_size*0.4],'visible','on');
%     subplot(1,3, 1); imagesc(stim_PP); axis equal; colorbar; title(['stim X=',num2str(stim_X1)])
%     subplot(1,3, 2); imagesc(attn_PP); axis equal; colorbar; title(['attn X=',num2str(attn_X1)])
%     subplot(1,3, 3); imagesc(norm_PP); axis equal; colorbar; title(['norm X=',num2str(norm_X1)])
%     temp_file_name = strcat('firing_rate_hits_',num2str(round(peak_activity_in(s, n, a), 1)), ...
%         '_passive_',num2str(round(peak_activity_out(s, n, a),1)),'.pdf');
% saveas(f, temp_file_name)
end    

stim_all = [stim_all, stim_amp_drift(s)] ;
norm_all = [norm_all, norm_drift(n)-1] ;
attn_all = [attn_all, attn_drift(a)] ;
...
%     '_ratio_',num2str(round(ratio,2)),'.pdf');
% saveas(f, temp_file_name)
