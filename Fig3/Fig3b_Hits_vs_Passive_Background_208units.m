...
AvP_units_id = SRF_shift_test_analysis.unitsBHvP;
N_units = size(AvP_units_id, 1); %>>>>>>>>>>>>>>207
rP_trial_mean = nan(N_units, 1);
rA_trial_mean = nan(N_units, 1);
AvsP_p_value = nan(N_units, 1);
AvP_units_raw = SRF_shift_test_analysis.background_hits_vs_passive;%************* Key Changes
id_rate_P = 1 ;
id_rate_A = 2 ;
id_mon = 30 ;
id_unit = 31 ;
for n = 1 : N_units
    ...
    temp_trial_id = find ((AvP_units_raw(:, id_mon)==mon_id)&(AvP_units_raw(:, id_unit)==unit_id));%************* Key Changes
