...
close all
CH = 4 ;
stim_select = 1 ;  % relative stimulus in stim_list; <=5
bin_ms = 10 ;
...
file_name_C='M9X0844';
[spikeTimes_C, spikeMat_C, ~, ~, ~, ~, ~] = spike_extract(file_name_C, CH);
...
subplot(1,3,2)
nrep = numel(spikeTimes_C{stim_select});
patch([0 stim_ms stim_ms 0], [0 0 nrep+1 nrep+1], [0.9 0.9 0.9],'EdgeColor', 'none', 'FaceAlpha', 0.25);hold on
for r = 1:nrep
    x = spikeTimes_C{stim_select}{r};
    if ~isempty(x)
        plot(x, r*ones(size(x)), 'k.', 'MarkerSize', 8); hold on
    end
end
xlim([t_ms(1) t_ms(end)]); ylim([0 nrep + 1]); xlabel('Time from stimulus onset (ms)'); ylabel('Repeat #');
title(sprintf('Control Location %d', stim_list(stim_select))); set(gca, 'Box', 'off');
%%
[t_msX, mean_rateX_bs, baseline_rate] = psth_plot(spikeMat, stim_select, bin_ms, pre_ms, stim_ms, post_ms, t_ms);
[~, mean_rateX_bs_C, baseline_rate_C] = psth_plot(spikeMat_C, stim_select, bin_ms, pre_ms, stim_ms, post_ms, t_ms);
...
plot(t_msX, mean_rateX_bs_C, 'b', 'LineWidth', 1.5);
