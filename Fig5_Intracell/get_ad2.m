%--------------------------------------------------------------------------
% Get AD2 Data (Modified to return data and header)
%--------------------------------------------------------------------------
function varargout = get_ad2(varargin)
%
% GET_AD2 Function
%
...
[ad2ver,nch,on_ch,nstim,nrep,sr,dur,ds,pre_stimulus_duration] = local_getheaderinfo(hdr);
...
%             if stim_point-floor((pre_stimulus_duration/1000)*new_sr(1)) < 1
%                 ad2data = [ad2data(1)*ones(1+floor((pre_stimulus_duration/1000)*new_sr(1))-stim_point,1); ad2data];
%                 stim_point = stim_point+(1+floor((pre_stimulus_duration/1000)*new_sr(1))-stim_point);
%             end
            ad2data = ad2data(stim_point-floor((pre_stimulus_duration/1000)*new_sr(1)):end); %cut off data before pre stim.
            ad2data = ad2data(1:round((dur(stim)/1000)*new_sr(1))); %cut off data after post stim.
        end
        catch
...
function [ad2ver,nch,on_ch,nstim,nrep,sr,dur,ds,varargout] = local_getheaderinfo(hdr)
